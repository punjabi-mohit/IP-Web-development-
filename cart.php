...
<?php
		...
		$query  = "SELECT * FROM cart where user_email='$user'";
		$result = $conn->query($query);
		$rows=$result->num_rows;
		if ($rows==0)
		{
			echo "<h1>Shopping Cart is Empty</h1>";
			echo "<p>Your Shopping Cart lives to serve. Give it purpose--fill it with books, CDs, videos, DVDs, electronics, and more. If you already have an account, Sign In to see your Cart.</p><br>
<p>Continue shopping on the Amazon.in homepage, learn about today's deals, or visit your Wish List.</p>
...
<p>Do you have a promotional code? We'll ask you to enter your claim code when it's time to pay.</p>";
		...
		echo"<p Style='padding-left:90%'> price</p> ";
		echo"<hr>";
		$total=0;
			for ($j = 0 ; $j < $rows ; ++$j)
			{
				...
				echo "<p style='float:left;width:80%'>$row[2] <p>₹$row[3]</p></p><hr style='width:100%'>";
				$row[3] = floatval(preg_replace('/[^\d.]/', '', $row[3]));
				$a=(float)$row[3];
				$total=$total+$a;
			}
			echo"<p style='padding-left:90%'>Total:₹$total</P>";
		echo"</div>";
		}
?>
